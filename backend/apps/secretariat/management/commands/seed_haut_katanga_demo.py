...
# Reference data — structure type RDC (démo plateforme)
...
# Profils anglophones (nom, postnom, prénom, sexe, nationalité, lieu de naissance)
ENGLISH_PROFILES = (
    ("Smith", "", "James", "M", "Britannique", "London"),
    ("Johnson", "", "Emily", "F", "Américaine", "New York"),
    ("Williams", "Lee", "Michael", "M", "Canadienne", "Toronto"),
    ("Brown", "", "Sarah", "F", "Britannique", "Manchester"),
    ("Taylor", "", "Daniel", "M", "Américaine", "Chicago"),
    ("Wilson", "", "Olivia", "F", "Australienne", "Sydney"),
    ("Thomas", "", "David", "M", "Britannique", "Birmingham"),
    ("Anderson", "", "Sophie", "F", "Américaine", "Boston"),
    ("Clark", "", "William", "M", "Néo-zélandaise", "Auckland"),
    ("Walker", "", "Grace", "F", "Irlandaise", "Dublin"),
)
...
class Command(BaseCommand):
    help = (
        "Remplace les données secrétariat par des années scolaires peuplées "
        "(cohortes réalistes pour démo plateforme ELIMU One)."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=2009,
            help="Première année scolaire (ex. 2024 → 2024-2025).",
        )
        parser.add_argument(
            "--to-year",
            type=int,
            default=2027,
            help="Dernière année scolaire inclusive (ex. 2027 → 2026-2027).",
        )
        parser.add_argument(
            "--students",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="Élèves à profil anglophone injectés dans l'année active (défaut: 0).",
        )
        parser.add_argument(
            "--skip-cards",
            action="store_true",
        ...
        parser.add_argument(
            ...
            type=int,
            default=8,
            help="Années récentes pour seed_discipline_demo (défaut: 8).",
        )
        parser.add_argument(
            "--discipline-days",
            type=int,
            default=16,
            help="Jours de présence simulés par année (défaut: 16).",
        )
        parser.add_argument(
            "--cards-from",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        english_count = options["english"]
        skip_cards = options["skip_cards"]
        skip_discipline = options["skip_discipline"]
        skip_finance = options["skip_finance"]
        discipline_years = options["discipline_years"]
        discipline_days = options["discipline_days"]
        ...
        school_years = tuple(
            ...
            for start in range(from_year, to_year)
        )
        if not school_years:
            self.stderr.write("Plage d'années vide (--from-year / --to-year).")
            return

        active_label = school_years[-1]["label"]

        card_jobs: list[tuple[AcademicYear, list[Student]]] = []
        cohort_state = CohortState()

        # o2switch: une seule grosse transaction provoque timeout / rollback total.
        with transaction.atomic():
            ...
            years = self._create_years(school_years, active_label)
        ...
        if english_count > 0:
            active_year = years[-1]
            self.stdout.write(
                f"Ajout de {english_count} élève(s) anglophone(s) — {active_year.label}…"
            )
            english_students = self._inject_english_students(
                active_year, english_count, actor, rng
            )
            if not skip_cards and active_year.label >= cards_from:
                card_jobs.append((active_year, english_students))

        if card_jobs:
            total = sum(len(s) for _, s in card_jobs)
            self.stdout.write(
                f"Generation des cartes eleves ({total} cartes, annees >= {cards_from})…"
            )
            done = 0
        ...
        if not skip_discipline:
            ...
            try:
                call_command(
                    "seed_discipline_demo",
                    years=discipline_years,
                    days_per_year=discipline_days,
                )
            except Exception as exc:  # noqa: BLE001
                self.stderr.write(f"Discipline seed ignoré: {exc}")

        self.stdout.write("Configuration des horaires de pointage (avant-midi / après-midi)…")
        from apps.discipline.services.schedule_service import ensure_default_attendance_schedules

        for year in years:
            if year.is_closed:
                continue
            ensure_default_attendance_schedules(
                academic_year=year,
                actor=actor,
                relaxed_end_time=True,
            )
        ...
        self.stdout.write(self.style.SUCCESS(self._summary(years, active_label)))
    ...
    def _purge(self) -> None:
        ...
        try:
            ParentAttendanceNoticeRead.objects.all().delete()
            ParentPushDevice.objects.all().delete()

            PaymentAllocation.objects.all().delete()
            Payment.objects.all().delete()
            StudentFeeObligation.objects.all().delete()
            FeeAmountChangeRequest.objects.all().delete()
            FeeRevisionRequest.objects.all().delete()
            FeeClassAmount.objects.all().delete()
            FeeApprovalHistory.objects.all().delete()
            FeeTarget.objects.all().delete()
            SchoolFee.objects.all().delete()
            ReceiptSequence.objects.all().delete()

            IncidentParticipant.objects.all().delete()
            DisciplinaryMeasure.objects.all().delete()
            ParentSummons.objects.all().delete()
            AbsenceJustification.objects.all().delete()
            ExitAuthorization.objects.all().delete()
            DisciplinaryIncident.objects.all().delete()
            StudentAttendanceRecord.objects.all().delete()
            ClassAttendanceSheet.objects.all().delete()
            AttendanceScanEvent.objects.all().delete()
            DailyAttendance.objects.all().delete()
            AttendanceSchedule.objects.all().delete()
    ...
    # -------------------------------------------------------------- structure
    def _create_years(
        self, school_years: tuple[dict, ...], active_label: str
    ) -> list[AcademicYear]:
    ...
    def _inject_english_students(
        ...
        from apps.secretariat.seed.demo_cohort import CohortState, create_guardian_for_seed

        state = CohortState()
        target_classes = list(
            SchoolClass.objects.filter(academic_year=year, level__code="7E").order_by("name")
        )
        if not target_classes:
            return []

        profiles = ENGLISH_PROFILES[:count]
        created: list[Student] = []
        for index, (nom, postnom, prenom, sexe, nationalite, lieu) in enumerate(profiles):
            klass = target_classes[index % len(target_classes)]
            state.student_seq += 1
            ...
            matricule = f"ENG{year.start_date.year}{state.student_seq:04d}"
            student = Student.objects.create(
                matricule=matricule,
                nom=nom,
                postnom=postnom,
                ...
                lieu_naissance=lieu,
                nationalite=nationalite,
                adresse=f"Avenue Lumumba n°{rng.randint(1, 120)}, Likasi",
                ancien_etablissement=rng.choice(
                    (
                ...
                date_admission=year.start_date + timedelta(days=rng.randint(0, 14)),
                statut=Student.Status.ACTIVE,
                is_active=True,
                is_archived=False,
                observations="Profil anglophone — démo plateforme.",
            )
            guardian = create_guardian_for_seed(
                ...
                sexe=rng.choice([Guardian.Gender.MALE, Guardian.Gender.FEMALE]),
            ...
            StudentGuardian.objects.create(
                student=student,
                guardian=guardian,
                lien_parente=StudentGuardian.Relationship.LEGAL_GUARDIAN,
                is_primary=True,
                is_emergency_contact=True,
                can_pickup=True,
                receives_notifications=True,
            ...
            Enrollment.objects.create(
                ...
                academic_year=year,
                school_class=klass,
                enrollment_number=f"INS{year.start_date.year}E{state.student_seq:04d}",
    ...
    def _summary(self, years: list[AcademicYear], active_label: str) -> str:
        return (
            ...
            f"Année active : {active_label} · Toutes les autres années : clôturées."
        )
